...
@st.cache_resource
def charger_modele_fertilisation():
    base = os.path.dirname(os.path.dirname(__file__))
    try:
        model   = joblib.load(os.path.join(base, "models", "model_fertilisation.pkl"))
        scaler  = joblib.load(os.path.join(base, "models", "scaler_fertilisation.pkl"))
        le_eng  = joblib.load(os.path.join(base, "models", "label_encoder_engrais.pkl"))
        le_cult = joblib.load(os.path.join(base, "models", "label_encoder_culture.pkl"))
        le_sol  = joblib.load(os.path.join(base, "models", "label_encoder_sol.pkl"))
        return model, scaler, le_eng, le_cult, le_sol, True
    except FileNotFoundError:
        return None, None, None, None, None, False

model_f, scaler_f, le_eng, le_cult, le_sol, ok = charger_modele_fertilisation()
...
if not ok:
    st.error(
        "⚠️ Modèle non trouvé. Placez les fichiers `.pkl` dans `models/`"
    )
    st.stop()
...
with col3:
    ...
    st.divider()
    st.info(
        f"**{culture}** — Sol {type_sol}\n\n"
        f"N={N:.0f} | P={P:.0f} | K={K:.0f} kg/ha"
    )
...
if st.button("🔍 Recommander un engrais", type="primary",
    ...
    with col_r1:
        ...
        st.markdown(
            ...
            f'<p style="margin:0.3rem 0 0;font-size:1rem">{info.get("nom","")}</p>'
            f'</div>',
    ...
    # Probabilités par engrais
    with st.expander("Distribution des probabilités par engrais"):
        classes = le_eng.classes_
