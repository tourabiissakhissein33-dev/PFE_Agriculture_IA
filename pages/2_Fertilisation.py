...
st.set_page_config(page_title="Fertilisation",page_icon="🌿",layout="wide",initial_sidebar_state="expanded")
ROOT=os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if ROOT not in sys.path: sys.path.insert(0,ROOT)
from styles import GLOBAL_CSS
from meteo_api import VILLES_TCHAD, get_meteo_actuelle
from database import sauvegarder_fertilisation

st.markdown(GLOBAL_CSS,unsafe_allow_html=True)
st.markdown("""<style>[data-testid="stAppViewContainer"]{background:linear-gradient(160deg,#e8f5e9 0%,#f1f8e9 100%)}</style>""",unsafe_allow_html=True)

@st.cache_resource
def charger():
    for base in [os.path.join(ROOT,"models"),os.path.join(os.getcwd(),"models"),"models"]:
        try:
            return (joblib.load(os.path.join(base,"model_fertilisation.pkl")),
                    ...
                    joblib.load(os.path.join(base,"label_encoder_sol.pkl")),True)
        except Exception: continue
    return None,None,None,None,None,False

model_f,scaler_f,le_eng,le_cult,le_sol,ok=charger()
ENGRAIS={
    "Uree":("Urée 46-0-0","50–100 kg/ha","Semis + 30j après levée","#1565c0"),
...
<div class="page-header" style="background:linear-gradient(135deg,#1b5e20,#27ae60)">
  ...
  <div><h1>Module Fertilisation</h1>
  <p>XGBoost · Accuracy 95.42% · F1-macro 95.47% · CV 97.23% · 1 599 obs. · Top variable : P (0.44)</p></div>
...
st.markdown('<div class="meteo-title">🛰️ Conditions climatiques temps réel — Open-Meteo</div>',unsafe_allow_html=True)
...
if btn_m or st.session_state.ville_fert!=ville_sel:
    ...
    with st.spinner(f"Météo {ville_sel}..."):
        c=VILLES_TCHAD[ville_sel]; st.session_state.meteo_fert=get_meteo_actuelle(c["lat"],c["lon"])
...
st.info("La température et l'humidité influencent l'absorption des nutriments et la volatilisation de l'azote (Urée).")
...
if meteo and meteo.get("ok"): st.success("✅ Température, humidité et pluie pré-remplies")
...
with col2:
    lbl="✅ Open-Meteo" if meteo and meteo.get("ok") else "⬆️ Charger météo"
    st.markdown(f'<div class="card"><div class="card-title">🌡️ Conditions climatiques <span style="color:#27ae60;font-weight:400;text-transform:none">{lbl}</span></div>',unsafe_allow_html=True)
    temperature=st.slider("Température (°C)",15.0,45.0,float(t_def),0.5)
    humidite_air=st.slider("Humidité air (%)",10.0,95.0,float(hu_def),1.0)
    humidite_sol=st.slider("Humidité sol (%)",5.0,90.0,40.0,1.0)
    pluie=st.number_input("Pluie récente (mm)",0.0,300.0,float(pl_def),5.0)
    st.markdown('</div>',unsafe_allow_html=True)
...
with col3:
    ...
    st.markdown(f'<div style="background:#f0f4f0;border-radius:8px;padding:.8rem;font-size:.82rem;margin-top:.5rem;line-height:1.8">🌾 <b>{culture}</b> · Sol {type_sol}<br>🗺️ Zone {zone} · 📍 {ville_sel}</div>',unsafe_allow_html=True)
    st.markdown('</div>',unsafe_allow_html=True)
...
if st.button("🔍 Recommander un engrais",type="primary",use_container_width=True):
    ...
    with col_r1:
        cl=info[3]
        st.markdown(
            f'<div style="background:white;border-left:5px solid {cl};border-radius:12px;'
            f'padding:1.3rem;box-shadow:0 2px 10px rgba(0,0,0,.07);margin-bottom:.8rem">'
            ...
            f'📏 <b>Dose :</b> {info[1]}<br>⏰ <b>Moment :</b> {info[2]}</div></div>',
            unsafe_allow_html=True)
        if meteo and meteo.get("ok"):
            st.info(f"🛰️ Contexte météo ({ville_sel}) : T°={temperature}°C · Hum={humidite_air}% · Pluie={pluie}mm")
    with col_r2:
        st.metric("Confiance",f"{conf:.1f}%")
        st.metric("Culture",culture)
        st.metric("Sol",type_sol)
        st.metric("Météo","Open-Meteo ✅" if meteo and meteo.get("ok") else "Manuel")
    ...
    with st.expander("ℹ️ Informations du modèle"):
        ...
        |---|---|
        | Accuracy | **95.42%** |
        | F1-Score (macro) | **95.47%** |
        | CV Score (k=5) | **97.23%** |
        | Observations | 1 599 (Kaggle + augmentation) |
        | Top variable | P = 0.44 |
        | 2e variable | N = 0.34 |
        | 3e variable | K = 0.05 |
        """)

    with st.expander("📊 Probabilités par engrais"):
    ...
    try:
        sauvegarder_fertilisation(ville=ville_sel,culture=culture,type_sol=type_sol,
            zone=zone,azote_n=N,phosphore_p=P,potassium_k=K,ph_sol=ph,
            temperature=temperature,humidite_air=humidite_air,pluie=pluie,
            engrais_recommande=engrais,confiance=conf)
    ...
    st.caption(f"XGBoost · Météo: {src} · Accuracy: 95.42% · F1: 95.47% · CV: 97.23%")
